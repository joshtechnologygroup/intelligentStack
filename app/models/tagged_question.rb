  ...
  def self.get_question(user_id)
    ...
    for tag in tags
      ...
      unless questions.blank?
        ...
        users.each do |user|
          User.generate_user_performance_data(1, 1, user) unless TagScore.find_by(user_id: user)
        end
        ...
        query = "
        INSERT INTO suggested_questions SELECT #{user_id}, title, question_id, link, (NOW()::DATE - TO_TIMESTAMP(creation_date)::DATE)*(-1) + COALESCE(b.score, 0)* #{score_weight} + COALESCE(answer_count, 0) * #{answers_weight} -
        COALESCE(max_score, 0) * #{max_score_weight} + COALESCE(favorite_count, 0) * #{favorites_weight} + COALESCE(up_vote_count,0)*#{upvote_weight}
        + COALESCE(down_vote_count, 0)*#{downvote_weight} as sort_column, NOW()::Date FROM ( select * from questions WHERE (tags LIKE '%#{tag_name}%' and accepted_answer_id is NULL) )a LEFT OUTER JOIN (select * from tag_scores where tag_id ='12') b on a.\"owner.user_id\"=b.user_id::BIGINT WHERE b.score is NULL or b.score <= #{TagScore.where(tag_id: tag.tag_id,user_id: user_id).first.score} ;
